...
if ( ! function_exists( 'blogger_setup' ) ) :
	...
	function blogger_setup() {
		 ...
		 * to change 'blogger' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'blogger' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'blogger_custom_background_args',
				array(
...
function blogger_scripts() {
	wp_enqueue_style( 'blogger-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'blogger-style', 'rtl', 'replace' );

	wp_enqueue_script( 'blogger-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
